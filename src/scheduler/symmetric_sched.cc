...
ncclResult_t ncclSymmetricTaskScheduler(struct ncclComm* comm,
  ...
  if (kernelId == ncclSymkKernelId_AllGather_RailRing_LsaSTMC && !plan->persistent) {
    ...
    plan->symAgGinProfileRecords = nProfileRecords;
    plan->symAgGinProfileBlocks = curChannel;
    plan->symAgGinProfileEventsPerPath = eventsPerPath;
    argsBuf->agGinProfile = plan->symAgGinProfileDev;
    argsBuf->agGinProfileRecords = (int)nProfileRecords;
    argsBuf->agGinProfileBlocks = curChannel;
    argsBuf->agGinProfileEventsPerPath = eventsPerPath;
  }
#endif
